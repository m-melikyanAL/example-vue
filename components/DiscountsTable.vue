...
const handleCheckAllChange = async () => {
  checkedItems.value = checkAll.value
    ? props.items.map(discount => discount.id)
    : []
}
...
const handleSingleDelete = async (id) => {
  renderPopup({
    ...
    message: ref(t('discounts.delete_message')),
    type: "error",
...
const handleBulkDelete = async () => {
  renderPopup({
    showing: true,
    heading: ref(t('alerts.heading')),
    message: ref(t('discounts.bulk_delete_message')),
    confirm_text: ref(t('alerts.confirm_text')),
    cancel_text: ref(t('buttons.cancel')),
    type: 'error',
    callback: () => {
  ...
  <table class="w-full table-auto max-w-6xl">
    <thead>
      <tr>
        ...
        <th>{{ $t('discounts.list.code') }}</th>
        <th>{{ $t('discounts.list.value_display') }}</th>
        <th>{{ $t('discounts.list.start_date') }}</th>
        <th>{{ $t('discounts.list.end_date') }}</th>
